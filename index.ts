...
function printHelp(): void {
  ...
  const lines = [
    ...
    `  ${t.brand("--preflight")}            run deployment/runtime checks and exit`,
    ...
    while (true) {
      ...
      if (slack) {
        ...
        if (!threadTs) {
          console.log(
            t.warn(
              `! slack: unable to post to channel (${slack.getLastError() || "unknown error"}). Make sure the bot is invited and channel ID is correct.`,
            ),
          );
        }
      }
...
async function main(): Promise<void> {
  ...
  if (flags.preflight) {
    const { runPreflight } = await import("./src/preflight");
  ...
  if (missingRequiredEnv().length > 0) {
    try {
      ...
      console.error(
        `\n${t.muted("you can manually populate .env and rerun. required keys:")} ${t.brand("OPENROUTER_API_KEY")}`,
      );
      process.exit(1);
    }
  }

  if (envBool("REQUIRE_DATABASE") && !process.env.DATABASE_URL?.trim()) {
    console.error(
      t.bad("DATABASE_URL is required because REQUIRE_DATABASE=true"),
    );
    process.exit(1);
  }

  const hasInput =
    flags.urls.length > 0 || !!flags.urlsFile || !!flags.schoolsCsv;
...
async function loadSchoolsCsv(path: string): Promise<SchoolCsvItem[]> {
  ...
  for (const r of rows) {
    ...
    const cand1Score =
      parseInt((r["Candidate 1 Score"] ?? "0").replace(/[^0-9-]/g, "")) || 0;
    const cand2Score =
      parseInt((r["Candidate 2 Score"] ?? "0").replace(/[^0-9-]/g, "")) || 0;
    const cand3Score =
      parseInt((r["Candidate 3 Score"] ?? "0").replace(/[^0-9-]/g, "")) || 0;
    const origin = (() => {
      try {
    ...
    items.push({
      ...
      city,
      url: homepage,
      ...(preferredDirectoryUrls.length > 0 ? { preferredDirectoryUrls } : {}),
    });
  }
  return items;
}
...
function statusCsvPath(): string {
  return resolve(process.env.STATUS_CSV_PATH?.trim() || "output/status.csv");
}
...
async function writeStatusCsv(
  ...
  const byOutput = new Map<string, BatchOutcome>();
  for (const outcome of outcomes) {
    if (outcome) byOutput.set(outcome.item.outputPath, outcome);
  }
  ...
  for (const item of items) {
    ...
    lines.push(
      [
        ...
        csvField(item.state ?? ""),
        csvField(done),
        csvField(teacherCount),
        csvField(status === "ok" ? "done" : status),
        csvField(item.outputPath),
        csvField(outcome?.error ?? ""),
      ].join(","),
    );
  }

  await writeCsv(path, lines.join("\n") + "\n");
}
...
async function runFromSchoolsCsv(flags: CliFlags): Promise<void> {
  ...
  if (schools.length === 0) {
    console.error(
      t.bad(
        "no valid rows in schools csv (need: Hs ID, Name, State, City, School Homepage, Primary URL/Candidates)",
      ),
    );
    process.exit(1);
  }
  ...
  const items: BatchItem[] = schools.map((s) => {
    ...
    const hsId = Number.parseInt(s.id, 10);
    return {
      url: s.url,
      outputPath,
      slug,
      ...
      city: s.city,
      state: s.state,
      ...(Number.isFinite(hsId) ? { hsId } : {}),
      ...(preferredDirectoryUrls.length > 0 ? { preferredDirectoryUrls } : {}),
    };
  });

  const statusPath = statusCsvPath();
  await writeStatusCsv(items, [], statusPath);

  console.log(
    `${BRAND_TAG}  ${t.muted(`${items.length} school${items.length === 1 ? "" : "s"}, concurrency ${concurrency}`)}`,
  );
  console.log(`${t.muted("status csv:")} ${t.brand(statusPath)}`);
  ...
  const outcomes = await runBatch(
    ...
    async (latest) => {
      statusWrite = statusWrite.then(() =>
        writeStatusCsv(items, latest, statusPath),
      );
      await statusWrite;
    },
  );
